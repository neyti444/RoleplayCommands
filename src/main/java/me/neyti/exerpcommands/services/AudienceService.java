package me.neyti.exerpcommands.services;

import me.neyti.exerpcommands.ExeRpCommands;
import me.neyti.exerpcommands.platform.SchedulerAdapter;
...
public final class AudienceService {
    ...
    public AudienceService(ExeRpCommands plugin) {
        this.plugin = plugin;
        this.scheduler = new SchedulerAdapter(plugin);
    }

    /** Сохранённый шорткат (на случай старых вызовов): radius >= 0, global=false. */
    public void sendToAudience(Player source, int radius, Consumer<Player> deliver) {
        sendToAudience(source, radius, false, deliver);
    }

    /** Основной метод: radius и флаг global задаёт команда. */
