...
public class WhisperCommand implements CommandExecutor {
    ...
    public WhisperCommand(ExeRpCommands plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.getConfigManager().isCommandEnabled("whisper")) {
        ...
        if (plugin.getConfigManager().isPermissionsEnabled()) {
            if (!player.hasPermission("exerpcommands.whisper")) {
        ...
        String format = plugin.getLanguageManager().getMessage("whisper");
        format = format.replace("{player}", player.getName())
                .replace("{message}", message);

        // Отправка сообщения по радиусу или всем
        if (plugin.getConfigManager().isRadiusEnabled()) {
            int radius = plugin.getConfigManager().getCommandRadius("whisper");
