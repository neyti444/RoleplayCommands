...
public class DoCommand implements CommandExecutor {
    ...
    public DoCommand(ExeRpCommands plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.getConfigManager().isCommandEnabled("do")) {
        ...
        if (plugin.getConfigManager().isPermissionsEnabled()) {
            if (!player.hasPermission("exerpcommands.do")) {
        ...
        String format = plugin.getLanguageManager().getMessage("do");
        format = format.replace("{player}", player.getName())
                .replace("{message}", message);

        // Отправка сообщения по радиусу или всем
        if (plugin.getConfigManager().isRadiusEnabled()) {
            int radius = plugin.getConfigManager().getCommandRadius("do");
