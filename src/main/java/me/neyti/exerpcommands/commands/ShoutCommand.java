...
public class ShoutCommand implements CommandExecutor {
    ...
    public ShoutCommand(ExeRpCommands plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (!plugin.getConfigManager().isCommandEnabled("shout")) {
        ...
        if (plugin.getConfigManager().isPermissionsEnabled()) {
            if (!player.hasPermission("exerpcommands.shout")) {
        ...
        String format = plugin.getLanguageManager().getMessage("shout");
        format = format.replace("{player}", player.getName())
                .replace("{message}", message);

        // Отправка сообщения по радиусу или всем
        if (plugin.getConfigManager().isRadiusEnabled()) {
            int radius = plugin.getConfigManager().getCommandRadius("shout");
