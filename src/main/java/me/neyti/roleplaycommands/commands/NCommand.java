...
public class NCommand extends AbstractMessageCommand {

    public NCommand(RoleplayCommands plugin) { super(plugin); }

    @Override protected String commandKey() { return "n"; }
    ...
    protected boolean executeFor(Player player, String[] args, int commandRadius, boolean global) {
        ...
        plugin.getAudienceService().sendToAudience(player, commandRadius, global, p ->
                plugin.getChatService().sendTemplate(p, plugin.getMessages().N, ph, player)
        );
        return true;
    }
}
