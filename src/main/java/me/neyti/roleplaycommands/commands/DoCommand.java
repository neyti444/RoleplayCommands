...
public class DoCommand extends AbstractMessageCommand {

    public DoCommand(RoleplayCommands plugin) { super(plugin); }

    @Override protected String commandKey() { return "do"; }
    ...
    protected boolean executeFor(Player player, String[] args, int commandRadius, boolean global) {
        ...
        plugin.getAudienceService().sendToAudience(player, commandRadius, global, p ->
                plugin.getChatService().sendTemplate(p, plugin.getMessages().DO, ph, player)
        );
        return true;
    }
}
